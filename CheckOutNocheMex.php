<?php
	...
	$query="UPDATE T_CheckIn SET F_NocheMex = -1 WHERE F_Id = '".$id."'";
	$checkInHotel=mysqli_query($con, $query);
	// Registra movimiento
	$query="INSERT INTO T_Movimientos VALUES ('".$id."', 'CHECK-OUT','1',NOW(), 'CHECK-OUT NOCHE MEXICANA')";
	$insertaMovimiento=mysqli_query($con, $query);

	$query="SELECT F_CenaGala FROM T_CheckIn WHERE F_Id = '".$id."'";
	$checkInHotel=mysqli_query($con, $query );
